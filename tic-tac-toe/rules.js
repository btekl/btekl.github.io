...
function begin_play(){
	...
	let player_1 = document.getElementById("player1_id");
	let player_2 = document.getElementById("player2_id");
	if(isEmpty(player_1.value) | isEmpty(player_2.value)){
...
function reset_play(){
	if(game_started()){
		...
		player_2.value = "";
		this.turn = 1;
		turn_info.innerHTML = "Game has not yet begun.";
...
function valid_move(move){
	for(var i = 0; i < table_ids.length; ++i){
		if(table_ids[i] === move && board_state[i] == -1){
			return true;
		}
	}
	alert("Invalid move entered");
	return false;
}

function set_board_state(move,player,marker){
	let board_space = document.getElementById(move);
	board_space.innerHTML = marker;
...
function check_winner(){
	...
	var winner = false;
	if((board_state[0] == board_marker) && (board_state[1] == board_marker) && (board_state[2] == board_marker)){
		winner = true;
	} else if((board_state[3] == board_marker) && (board_state[4] == board_marker) && (board_state[5] == board_marker)){
		winner = true;
	} else if((board_state[6] == board_marker) && (board_state[7] == board_marker) && (board_state[8] == board_marker)){
		winner = true;
	} else if((board_state[0] == board_marker) && (board_state[3] == board_marker) && (board_state[6] == board_marker)){
		winner = true;
	} else if((board_state[1] == board_marker) && (board_state[4] == board_marker) && (board_state[7] == board_marker)){
		winner = true;
	} else if((board_state[2] == board_marker) && (board_state[5] == board_marker) && (board_state[8] == board_marker)){
		winner = true;
	} else if((board_state[0] == board_marker) && (board_state[4] == board_marker) && (board_state[8] == board_marker)){
		winner = true;
	} else if((board_state[2] == board_marker) && (board_state[4] == board_marker) && (board_state[6] == board_marker)){
		winner = true;
	}
	if(winner){
...
function play() {
	...
	let move_choice = document.getElementById("move_text_id").value;
	let turn_info = document.getElementById("turn_info");

	if(valid_move(move_choice)){
		if(this.turn){ // Player X
			set_board_state(move_choice,1,"X")
			turn_info.innerHTML = "Turn for: <b>O</b>";
		} else{ // Player O
			set_board_state(move_choice,2,"O")
			turn_info.innerHTML = "Turn for: <b>X</b>";
		}
		check_winner();
		toggle_move();
